...
| `gharchive_events_total` | Total events of all types. A general activity level signal — completely inactive plugins are far more likely to have unpatched vulnerabilities. |
| `gharchive_push_events` | Number of push events (direct commits to any branch). High push frequency suggests active development; very low values over multiple months indicate a stale codebase. |
| `gharchive_pull_request_events` | Number of PR-related events (opened, closed, reopened). PR activity indicates a collaborative review workflow rather than solo direct pushes. |
...
| `gharchive_pull_request_review_events` | Review events (approvals, change requests, comments). A direct signal of code review activity — projects with active review catch more bugs before they ship. |
...
| `gharchive_release_events` | GitHub release publications. Regular releases indicate a mature, disciplined release process. Infrequent releases may mean security fixes sit in code for longer before reaching users. |
...
| `gharchive_days_active` | Number of distinct calendar days with at least one event. Spread-out activity suggests professional maintenance; clustering on a few days suggests hobbyist or part-time maintenance. |
| `gharchive_watch_events` | GitHub star events received this month. A proxy for community interest and visibility. High-visibility projects receive more scrutiny (both helpful and adversarial). |
| `gharchive_fork_events` | Fork events this month. Forks indicate downstream users and potential contributors; a large fork count also suggests the codebase is being studied and adapted. |
...
| `gharchive_bot_events` | Events attributable to known bot accounts (Dependabot, Renovate, GitHub Actions, etc.). Bot-driven activity indicates automated tooling is in place. |
| `gharchive_human_events` | Events from human (non-bot) actors. Complements `bot_events` — a repo where only bots are active may be effectively abandoned by human maintainers. |
| `gharchive_unique_human_actors` | Distinct human (non-bot) actors this month. A purer bus-factor signal than `unique_actors` since it excludes automation noise. |
| `gharchive_owner_push_fraction` | Fraction of push events from the single most active human pusher. High values (close to 1.0) indicate one person controls the codebase — a high bus-factor risk. |
| `gharchive_security_keyword_events` | PR or issue events whose title/body contains security-related keywords (CVE, vulnerability, exploit, RCE, XSS, injection, etc.). A direct leading indicator of known or suspected security issues being discussed in the project. |
...
| `gharchive_dependency_bump_events` | PRs or issues related to dependency updates (Dependabot PRs, "bump" commits, Renovate). Projects that actively update dependencies reduce their exposure window to known vulnerable dependencies. |
| `gharchive_pr_merge_time_p50_hours` | Median hours from PR creation to merge. Longer merge times suggest either a thorough review process (positive) or a bottlenecked/understaffed team (negative). Extremely long or bimodal distributions warrant investigation. |
| `gharchive_pr_merge_time_p90_hours` | 90th percentile of PR merge time. The tail captures how long the slowest PRs take — a high p90 may indicate security-sensitive PRs are getting stuck. |
| `gharchive_issue_close_time_p50_hours` | Median hours from issue creation to close. Fast issue resolution suggests an engaged maintainer team; very slow times may indicate security reports go unacknowledged. |
...
| Field | Predictive rationale |
|-------|----------------------|
| `gharchive_months_since_push` | Months since the last push event. One of the strongest staleness signals — projects that have not had a commit in many months are very unlikely to receive timely security patches. |
...
| `gharchive_months_since_release` | Months since the last GitHub release. Even if code is being committed, a project that never cuts releases may not be delivering security fixes to users. |
...
| `gharchive_months_since_security_keyword` | Months since a PR or issue mentioned a security keyword. Long gaps may mean security issues are not being discussed; very short gaps may indicate active security work. |
...
| Field | Predictive rationale |
|-------|----------------------|
| `gharchive_push_events_trailing_3m_delta_prev_3m` | Change in push volume between the current 3-month window and the previous 3-month window. A large negative delta may indicate a project is winding down. |
| `gharchive_pull_request_events_trailing_3m_delta_prev_3m` | Change in PR volume. A sudden drop in PR activity may signal contributor loss or project abandonment. |
| `gharchive_release_events_trailing_3m_delta_prev_3m` | Change in release frequency. Useful for detecting when a project that was regularly releasing suddenly stops. |
| `gharchive_security_keyword_events_trailing_3m_delta_prev_3m` | Change in security-keyword event volume. A spike may precede a public vulnerability disclosure. |
| `gharchive_watch_events_trailing_3m_delta_prev_3m` | Change in star velocity. A sudden spike in stars can indicate the project gained attention (positive) or was featured in a security advisory or news story (potentially negative). |
...
| Field | Predictive rationale |
|-------|----------------------|
| `gharchive_prs_per_push_3m` | PRs per push event over 3 months. High values indicate most code changes go through PR review rather than direct push. A strong positive signal for code review culture. |
...
| `gharchive_pr_review_intensity_3m` | Review events per PR over 3 months. Higher values mean more review comments per PR, suggesting more thorough scrutiny of incoming code. |
...
| `gharchive_actors_per_active_day_3m` | Average distinct actors per active day over 3 months. Measures contributor density — projects with many contributors on each active day have healthier community dynamics. |
| `gharchive_actors_per_active_day_6m` | Same over 6 months. |
| `gharchive_active_month_ratio_3m` | Fraction of the last 3 months with at least one event. A value of 1.0 means the project was active every month; 0.33 means only one of the three months saw activity. |
...
| `gharchive_events_per_active_month_6m` | Events per active month over 6 months. A measure of activity intensity in the months when the project is actually being worked on. |
| `gharchive_activity_burstiness_6m` | Ratio of the peak month's activity to the 6-month average. High burstiness (value much greater than 1.0) indicates erratic, sprint-then-stall development patterns. Low burstiness indicates steady, predictable maintenance. |
| `gharchive_bot_event_ratio_3m` | Fraction of all events over 3 months that came from bot accounts. A high ratio with low human activity may indicate the project is effectively on autopilot — dependency bumps happening automatically but no human review. |
| `gharchive_security_keyword_rate_3m` | Security keyword events as a fraction of total PR+issue events over 3 months. Normalizes for project size — a project with 100 total events and 5 security keyword events is more concerning than one with 1000 events and 5 security keyword events. |
