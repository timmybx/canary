"""
Behavior tests for canary.collectors.plugin_snapshot.

Consolidates test_plugin_snapshot.py + test_plugin_snapshot_extra.py.
"""
...
# ---------------------------------------------------------------------------
# collect_plugin_snapshot — fixture-backed real=True test
# ---------------------------------------------------------------------------
...
def test_fetch_plugin_api_json_success(monkeypatch):
    ...
    assert _fetch_plugin_api_json("myplugin")["name"] == "myplugin"
...
def test_extract_historical_plugin_ids_previous_names_list():
    assert _extract_historical_plugin_ids({"previousNames": ["old-name-1", "old-name-2"]}) == [
        "old-name-1",
        "old-name-2",
    ]
...
def test_extract_historical_plugin_ids_previous_names_string():
    assert _extract_historical_plugin_ids({"previousNames": "single-old-name"}) == [
        "single-old-name"
    ]


def test_extract_historical_plugin_ids_aliases():
    assert _extract_historical_plugin_ids({"aliases": ["alias-1", "alias-2"]}) == [
        "alias-1",
        "alias-2",
    ]
...
def test_extract_historical_plugin_ids_deduplicates():
    result = _extract_historical_plugin_ids(
        {"previousNames": ["dup-name"], "aliases": ["dup-name", "other-name"]}
    )
...
def test_extract_historical_plugin_ids_skips_empty_strings():
    assert _extract_historical_plugin_ids({"previousNames": ["", "  ", "valid-name"]}) == [
        "valid-name"
    ]
...
def test_extract_historical_plugin_ids_multiple_keys():
    result = _extract_historical_plugin_ids(
        {"previousNames": ["prev-1"], "formerNames": ["former-1"], "legacyNames": ["legacy-1"]}
    )
...
def test_collect_plugin_snapshot_sample_plugin_api_is_none():
    assert collect_plugin_snapshot(plugin_id="cucumber-reports", real=False)["plugin_api"] is None
...
def test_collect_plugin_snapshot_real_enriches_fields(monkeypatch):
    ...
    monkeypatch.setattr("canary.collectors.github_repo.parse_github_owner_repo", lambda url: None)
...
def test_collect_plugin_snapshot_real_with_github_signals(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda owner, repo, timeout_s=15.0: {
            "stargazers_count": 42,
            "forks_count": 10,
            "open_issues_count": 5,
            "pushed_at": "2024-01-01T00:00:00Z",
    ...
    monkeypatch.setattr(
        "canary.collectors.github_repo.fetch_github_open_pulls", lambda *a, **kw: []
    )
    monkeypatch.setattr(
...
def test_collect_plugin_snapshot_real_github_repo_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "canary.collectors.github_repo.fetch_github_repo",
        lambda owner, repo: (_ for _ in ()).throw(RuntimeError("rate limited")),
    )
