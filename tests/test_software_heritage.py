...
def test_normalize_origin_url_strips_git_suffix():
    assert (
        _normalize_origin_url("https://github.com/org/repo.git")
...
def test_normalize_origin_url_strips_trailing_slash():
    assert (
        _normalize_origin_url("https://github.com/org/repo/")
        == "https://github.com/org/repo"
    )
...
def test_collect_real_reads_cached_files_without_http(tmp_path: Path, monkeypatch):
    ...
    (out_dir / f"{slug}.swh_latest_visit.json").write_text(
        '{"snapshot": null}', encoding="utf-8"
    )
