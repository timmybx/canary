...
def _write_index(out_dir: Path, plugin_id: str, index: dict) -> None:
    ...
    (out_dir / f"{plugin_id}.github_index.json").write_text(json.dumps(index), encoding="utf-8")
...
def test_collect_github_plugin_fetches_and_writes(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("canary.collectors.github_plugin.fetch_github_open_issues", fake_fetch_list)
    monkeypatch.setattr("canary.collectors.github_plugin.fetch_github_open_pulls", fake_fetch_list)
    monkeypatch.setattr(
