"""
Behavior tests for canary.collectors.github_repo.

Consolidates test_github_repo.py + test_github_repo_extra.py
+ test_github_repo_helpers.py.
"""
...
def _make_urlopen_mock(payload, headers_dict=None):
    """Context-manager mock that yields a response with the given JSON payload."""
...
def test_fetch_github_workflows_dir_via_urlopen_returns_list(monkeypatch):
    """Low-level check: a real HTTP 200 response yields a list."""
    payload = [
...
def test_fetch_github_workflows_dir_via_urlopen_returns_none_on_404(monkeypatch):
    """Low-level check: HTTP 404 maps to None."""
    err = urllib.error.HTTPError(
...
def test_fetch_github_workflows_dir_via_urlopen_reraises_non_404(monkeypatch):
    """Low-level check: HTTP 403 is re-raised."""
    err = urllib.error.HTTPError(
        url="https://api.github.com/repos/org/repo/contents/.github/workflows",
        code=403,
        msg="Forbidden",
