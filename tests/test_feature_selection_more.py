...
from canary.train.feature_selection import compute_shap_global_importance, run_feature_selection


def _mock_pipeline(model_step: object, imputer_step: object | None = None) -> MagicMock:
...
def test_run_feature_selection_records_success_failure_and_full_baseline(
    ...
    pipeline = _mock_pipeline(DummyClassifier(strategy="prior"))
    rows = [
...
def test_run_feature_selection_random_forest_skips_subset_retraining(
    ...
    pipeline = _mock_pipeline(RandomForestClassifier())
    ...
    monkeypatch.setattr(
        fs,
        "_rows_to_matrix",
        lambda *_args: SimpleNamespace(values=np.zeros((1, 1))),
    )
    monkeypatch.setattr(
        fs,
        "compute_shap_global_importance",
