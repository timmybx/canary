"""
Behavior tests for canary.collectors.plugins_registry.

Consolidates test_plugins_registry.py + test_plugins_registry_extra.py.
"""
...
def test_plugin_to_registry_record_basic():
    rec = _plugin_to_registry_record({"name": "cucumber-reports", "title": "Cucumber Reports"})
...
def test_plugin_to_registry_record_previous_names_key():
    rec = _plugin_to_registry_record({"name": "test-plugin", "previous_names": ["old-name"]})
    assert rec is not None
...
def test_registry_real_paginates_and_is_resilient_to_payload_shape(monkeypatch):
    pages = [
        {"plugins": [{"name": "a", "title": "A"}, {"name": "b", "title": "B"}], "total": 3},
        {"plugins": [{"name": "c", "title": "C"}], "total": 3},
    ]
    calls = {"n": 0}
...
def test_collect_plugins_registry_real_follows_next_link(monkeypatch):
    ...
    page2 = {"plugins": []}
    monkeypatch.setattr(
        "canary.collectors.plugins_registry._fetch_json", _make_fake_fetch([page1, page2])
    )
...
def test_collect_plugins_registry_real_list_payload(monkeypatch):
    payload = [{"name": "list-plugin-a"}, {"name": "list-plugin-b"}]
    monkeypatch.setattr(
        "canary.collectors.plugins_registry._fetch_json", _make_fake_fetch([payload])
    )
...
def test_collect_plugins_registry_real_max_plugins(monkeypatch):
    page = {"plugins": [{"name": f"plugin-{i}"} for i in range(10)], "total": 100}
    monkeypatch.setattr("canary.collectors.plugins_registry._fetch_json", _make_fake_fetch([page]))
...
def test_collect_plugins_registry_real_skips_non_dict_plugins(monkeypatch):
    page = {"plugins": [{"name": "good-plugin"}, "not-a-dict", 42, None], "total": 4}
    monkeypatch.setattr("canary.collectors.plugins_registry._fetch_json", _make_fake_fetch([page]))
...
def test_fetch_json_success(monkeypatch):
    ...
    assert _fetch_json(_VALID_URL) == payload
