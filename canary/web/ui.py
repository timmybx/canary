# ruff: noqa: E501
"""Presentation layer for the CANARY web console: CSS and HTML renderers.
...
# ---------------------------------------------------------------------------
# ML tab renderers (pure presentation; callers load and pass all data)
# ---------------------------------------------------------------------------
...
def _render_ml_metrics(
    metrics: dict[str, Any] | None, pk_data: dict[str, Any] | None = None
) -> str:
    if not metrics:
...
def _render_ml_tab(
    ...
    fs_data: dict[str, Any] | None = None,
    ...
    Pure presentation: precision@k (pk_data) and feature-selection (fs_data)
    payloads are loaded by the caller and passed in.
    """
    ...
    output_parts.append(
        ...
        + status_text
        + _render_ml_metrics(metrics, pk_data=pk_data)
        + "</section>"
    )
